...
function drawFrame() {
    ...
    // seems fine? sure hope so
    var color = window.getComputedStyle(canvas).getPropertyValue("color");
    ctx.fillStyle = color;
    ctx.beginPath();
...
function calcOffset(x1, y1, x2, y2) {
    ...
    const curve = magnitude * (Math.pow(1.4, -Math.pow(magnitude/(scale * 120), 2)) * 0.3 - (0.01 * scale));
